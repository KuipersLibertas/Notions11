...
//        $linkType = intval($request->input('linkType', 0));
        ...
        $isExists = FileListUser::where('id', '<>', $id)
            ->where('passdrop_url', $link)
            ->get()
            ->count();
        if ($isExists) {
            return response()->json([
...
//        $data->link_type  = config('constants.link_type')[$linkType];
    ...
    public function getLinkDetail(Request $request): JsonResponse {
        ...
        $url = trim($request->input('url'));
        $linkInfo = FileListUser::where('passdrop_url', $url)
            ->where(function ($query) {
                $query->where('expire_count', '>', 'download_count')
                    ->orWhere('expire_count', '=', 0);
            })
            ->where(function ($query) {
                $query->where('expires_on', '>', DB::raw('curdate()'))
        ...
        $ignoreValidate = strlen($linkInfo->passdrop_pwd) === 0;
        if ($ignoreValidate) {
            $ipAddress = $request->ip();
            $isPassdropitRequest = $request->is('api/'.config('app.api-version').'/passdropit/*');
            $this->validationProcess($linkInfo->id, '', $ipAddress, $isPassdropitRequest);
        }
    ...
    public function validateLink(Request $request): JsonResponse {
        ...
        $ipAddress = $request->ip();
        $isPassdropitRequest = $request->is('api/'.config('app.api-version').'/passdropit/*');
        $bl = $this->validationProcess($linkId, $password, $ipAddress, $isPassdropitRequest);
        if ($bl) {
            return response()->json([
                'success' => true
            ]);
    ...
    protected function validationProcess($id, $password, $ipAddress, $isPassdropitRequest): bool {
        $loggedInUser = auth('sanctum')->user();

        if (strlen($password) > 0) {
            $linkInfo = FileListUser::where('id', $id)
